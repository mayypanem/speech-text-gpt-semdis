...
def live_plotter(x_vec,y1_data,line1,identifier='',pause_time=0.1):
    ...
    # Clear previous annotations before adding new ones
    for txt in line1.axes.texts:
        txt.remove()

    # Annotate each data point
    for i in range(len(x_vec)):
        line1.axes.annotate(
            f'{y1_data[i]:.2f}',  # Format label to 2 decimal places
            (x_vec[i], y1_data[i]),  # Position (x, y)
            textcoords="offset points", 
            xytext=(10,0),  # Offset above the point
